...
//#define StringToTCHAR(str) (FString FStrTemp(str.c_str());{const TCHAR* TCHARTemp = *(FStrTemp);TCHARTemp;})

bool FStereoPanoramaManager::CombineAtlasesOnOutput = true;
//Slice Controls
...
IConsoleVariable* FStereoPanoramaManager::StepCaptureHeight			 = IConsoleManager::Get().RegisterConsoleVariable(TEXT("SP.StepCaptureHeight"), 2048, TEXT("The final spherical atlas height"), ECVF_Default);
...
void FStereoPanoramaManager::Start(const TArray<FString>& Args) {
	if (!ValidateRendererState())
	{
		return;
	}
	//for (int32 count = 0; count < Args.Num(); ++count) {
	//	//FString x(*Args[count]);
	//	
	//	string zz(TCHAR_TO_UTF8(*Args[count]));
	//}
	// Fucking UE FString
	vector<string> argsStr;
	for (int index=0; index < Args.Num(); ++index) {
		argsStr.emplace_back(TCHAR_TO_UTF8(*Args[index]));
	}
	setArgsMap(argsStr);
	//setArgsMap(Args);
	// STL string conver to FString and TCHAR
	/*FString StringToFStr((resolutionMap[argsMap["-r"]].begin())->c_str());
	const TCHAR* FStringToTCHAR = *StringToFStr;
	FStereoPanoramaManager::StepCaptureWidth->Set(FStringToTCHAR);*/

	// *(resolutionMap[argsMap["-r"]].begin()) is a FString Object
	// **(resolutionMap[argsMap["-r"]].begin()) is a TCHAR* Object
	FStereoPanoramaManager::StepCaptureWidth->Set(*((resolutionMap[argsMap["-r"]])[0]));
	FStereoPanoramaManager::StepCaptureHeight->Set(*((resolutionMap[argsMap["-r"]])[1]));

	// argsMap["-p"] is a FString Object
	// *argsMap["-p"] is a TCHAR* Object
	FStereoPanoramaManager::OutputDir->Set(*(argsMap["-p"]));
	
	// Using input Arg(-bo) Change CombineAtlasesOnOutput(bool value) to control binoculus output
	init_CombineAtlasesOnOutput();
	// Using input Arg(-b and -e) control begin Frame and end Frame
	/*StartFrame = FCString::Atoi(*argsMap["-b"]);
	EndFrame = FCString::Atoi(*argsMap["-e"]);
	...
	FStereoPanoramaManager::PanoramicScreenshot(FCString::Atoi(*argsMap["-b"]), FCString::Atoi(*argsMap["-e"]), EmptyDelegate);
}
...
void FStereoPanoramaManager::init_CombineAtlasesOnOutput() {
	if (FCString::Atoi(*argsMap["-bo"]) > 0) {
		FStereoPanoramaManager::CombineAtlasesOnOutput = true;
	}
	else {
		FStereoPanoramaManager::CombineAtlasesOnOutput = false;
	}
}


void FStereoPanoramaManager::DisableBinoculusOutput(const TArray<FString>& Args) {
	argsMap["-bo"] = "0";
	FStereoPanoramaManager::CombineAtlasesOnOutput = false;
}